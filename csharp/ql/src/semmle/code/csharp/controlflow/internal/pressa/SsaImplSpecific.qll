...
ControlFlowElement getBasicBlockNode(BasicBlock bb, int i) { result = bb.getElement(i) }

BasicBlock getImmediateDominator(BasicBlock bb) { result.immediatelyDominates(bb) }
...
class ReadKind extends TReadKind {
  string toString() {
    this = ActualRead() and result = "ActualRead"
    or
    this = OutRefExitRead() and result = "OutRefExitRead"
  }
}
