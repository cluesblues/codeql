...
private module Cached {
  ...
  private predicate adjacentVarReaches(
    Definition def, ControlFlow::BasicBlock bb1, int i1, ControlFlow::Node cfn
  ) {
    adjacentVarRead(def, bb1, i1, cfn)
    or
    exists(ControlFlow::BasicBlock bb2, int i2 |
      adjacentVarReaches(def, bb1, i1, bb2.getNode(i2)) and
      variableRead(bb2, i2, _, any(ReadKind rk | rk.isPseudo())) and
  ...
  private predicate adjacentVarActualRead(
    ...
    adjacentVarReaches(def, bb1, i1, cfn) and
    exists(ControlFlow::BasicBlock bb2, int i2 |
      cfn = bb2.getNode(i2) and
  ...
  predicate firstReadSameVar(Definition def, ControlFlow::Node cfn) {
    exists(ControlFlow::BasicBlock bb1, int i1 |
      def.definesAt(_, bb1, i1) and
      adjacentVarActualRead(def, bb1, i1, cfn)
    )
  }
  ...
  predicate adjacentReadPairSameVar(Definition def, ControlFlow::Node cfn1, ControlFlow::Node cfn2) {
    exists(ControlFlow::BasicBlock bb1, int i1 |
      ...
      adjacentVarActualRead(def, bb1, i1, cfn2)
    )
  }

  private ControlFlow::Node pseudoRead(ControlFlow::BasicBlock bb, int i) {
    variableRead(bb, i, _, any(ReadKind rk | rk.isPseudo())) and
    result = bb.getNode(i)
  }
  ...
  private predicate reachesLastRefExit(Definition def, ControlFlow::BasicBlock bb, int i) {
    ...
    exists(ControlFlow::BasicBlock bb0, int i0 |
      ...
      adjacentVarRead(def, bb, i, pseudoRead(bb0, i0))
    )
  }
