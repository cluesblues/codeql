...
private predicate localFlowPhiInput(DataFlowNode input, Ssa::PhiNode phi) {
  exists(Ssa::Definition def, BasicBlock bb, int i |
    SsaImpl::lastRefRedef(def, bb, i, phi) and
    def = phi.getAnInput()
  |
  ...
  exists(Ssa::PhiNode mid, BasicBlock bb, int i |
    ...
    SsaImpl::lastRefRedef(mid, bb, i, phi) and
    mid.definesAt(_, bb, i) and
    mid = phi.getAnInput()
  )
}
...
private predicate localExactStep(DataFlowNode src, DataFlowNode sink) {
  ...
  exists(BasicBlock bb1, int i1 |
    src = bb1.getNode(i1) and
    SsaImpl::adjacentVarRead(_, bb1, i1, sink)
  )
  or
...
private class ParameterDef extends VariableUpdate, Parameter {
  override Parameter getSource() { result = this }

  override Parameter getVariable() { result = this }
